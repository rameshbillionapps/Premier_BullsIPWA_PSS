...
ssp.webdb.setSalesItemBull = function (item, itemname, price1, price2, price3, roycost, frzdat, type) {
    ...
    $('#breedcustsupp').removeClass('green-keyword');
...
ssp.webdb.resetSalesItemBull = function (bullcustsupp) {
    ...
    if (bullcustsupp == 1) {
        $('#breedcustsupp').attr('class', 'breeddisc green-keyword');
        bullcustsupp = 0;
    } else {
        $('#breeddisc1').attr('class', 'breeddisc green-keyword');
        bullcustsupp = 0;
...
ssp.webdb.orderAddBull = function () {
    ssp.webdb.db.transaction(function (tx) {
        ...
        //}        
            ...
            tx.executeSql('INSERT INTO tblSales (SIACT,SIORNM,SIDATO,SITYPS,SICOD,SIQTY,SINAM,SIPRC,SIREP,SIANM,SIRETL,SIPGA,SITYPI,SIOTH,SILVL4,SILIN,SILVL1,SILVL2,SILVL3,SILVL5,SICOW,SIARM,SISUP,FRZDAT,LOTNOT,MOD,MODSTAMP,BREEDTYPE,FRZYYYYMMDD) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [$('#acctid').html(), $('#ordernum').html(), currenttime, bulltype, bullcode, bullqty, bullname, bullprice, window.localStorage.getItem("ssp_TechID"), $('#custname').html(), bullretail, (($('#chkPGA').is(':checked')) ? 1 : 0), prevdate, salescode, psprevbullsessfallcredit, prevtechid, bullmisc, taxrate1, taxrate2, psservicecall, cownote, bulllabor, bullcustsupp, bullfreeze, bullnote, 1, currenttime, beefon, bullfreezeYYYYMMDD], ssp.webdb.resetSalesItemBull(bullcustsupp), ssp.webdb.onError);        //JKS040218***ADDED BREEDTYPE ? breeding***
...
function loadOrderContentBlock(INVNUM, CANEDIT) {
    ...
    $('#tab-bulls').onTabShow(function () {
        ...
        $('#lookuplist').html('');
        ssp.webdb.getBulls('%');
        ssp.webdb.resetSalesItemBull(breedcustsupp);
